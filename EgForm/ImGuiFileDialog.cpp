...
#else
//	strncpy(vBuffer, str.c_str(), len);  // 수정
	strncpy_s(vBuffer, vBufferLen, str.c_str(), len);
#endif
	vBuffer[len] = '\0';
}
	...
	if (n >= 0)
	{
		for (i = 0; i < n; i++)
		{
			...
			if (infos.type == 'f')
			{
				...
				{
					m_FileList[m_FileList.size() - 1].ext = infos.fileName.substr(lpt);
...
/*!
*	\brief 파일 선택 대화상자를 오픈한다.
*
*	\param vName[in]			대화상자의 이름(예, "File Open", "File Save" 등)
*	\param vFilters[in]			파일의 필터링을 위한 확장자명
*	\param vPath[in]			검색할 디렉터리 ("." 이면 현재 디렉터리)
*	\param vDefaultFileName[in]	검색할 파일 이름 ("" 이면 지정된 확장자명을 가진 모든 파일)
*
*	\return 
*/
