...
protected:
    ...
    static void compute_rotation(const Scalar& x, const Scalar& y, Scalar& r, Scalar& c, Scalar& s)
    {
        ...
        const Scalar xsign = Scalar((x > Scalar(0)) - (x < Scalar(0)));
        const Scalar ysign = Scalar((y > Scalar(0)) - (y < Scalar(0)));
        const Scalar xabs = Scalar(x * xsign);
        const Scalar yabs = Scalar(y * ysign);
        if(xabs > yabs)
        {
            // In this case xabs != 0
